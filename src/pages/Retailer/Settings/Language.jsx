...
const languagesArr = [
    { name: 'English (Global)', code: 'en-US', active: true, region: 'International Commerce' },
    { name: 'Yoruba (Nigeria)', code: 'yo', active: false, region: 'Regional Merchant' },
    { name: 'Igbo (Nigeria)', code: 'ig', active: false, region: 'Regional Merchant' },
    { name: 'Hausa (Nigeria)', code: 'ha', active: false, region: 'Regional Merchant' },
    { name: 'Português (Brasil)', code: 'pt', active: false, region: 'Latin America' },
    { name: 'Italiano (Italia)', code: 'it', active: false, region: 'European Union' }
];

export default function RetailerLanguage() {
    const navigate = useNavigate();
    const [selected, setSelected] = useState('en-US');
    const [searchTerm, setSearchTerm] = useState('');

    const filteredLanguages = languagesArr.filter(lang =>
        ...
        <div className="app-container" style={{ padding: '0', backgroundColor: '#FBFCFC', minHeight: '100vh', paddingBottom: '7rem', position: 'relative', overflowX: 'hidden' }}>
            ...
            <header style={{ padding: '1.5rem', backgroundColor: 'white', display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 2px 15px rgba(0,0,0,0.03)', position: 'sticky', top: 0, zIndex: 100 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    <h1 style={{ fontSize: '1.1rem', fontWeight: '950', color: 'var(--primary)', letterSpacing: '-0.03em', textTransform: 'uppercase' }}>Merchant Localization</h1>
                </div>
            ...
            <div style={{ padding: '3.5rem 2rem' }}>
                <div style={{ marginBottom: '5rem', textAlign: 'center', position: 'relative', zIndex: 1 }}>
                    <motion.div
                    ...
                    >
                        <Globe size={56} color="var(--primary)" />
                        <motion.div
                            animate={{ rotate: 360, scale: [1, 1.2, 1] }}
                            transition={{ duration: 15, repeat: Infinity, ease: 'linear' }}
                            style={{ position: 'absolute', inset: -15, borderRadius: '56px', border: '2.5px dashed var(--primary)', opacity: 0.2 }}
                        />
                    </motion.div>
                    <h2 style={{ fontSize: '2.8rem', fontWeight: '950', color: 'var(--black)', marginBottom: '1.25rem', letterSpacing: '-0.07em', lineHeight: '1' }}>Command Interface <br /><span style={{ color: 'var(--primary)' }}>Localization.</span></h2>
                    <p style={{ fontSize: '1.2rem', color: '#6A7A7A', lineHeight: '1.7', fontWeight: '750', maxWidth: '340px', margin: '0 auto' }}>
                        Choose the linguistic framework for your merchant dashboard and logistics reporting.
                    </p>
                </div>

                {/* Advanced Search Matrix Architecture */}
                <div style={{ position: 'relative', marginBottom: '4.5rem', zIndex: 1 }}>
                ...
                {/* Registry Options Ledger Architecture */}
                ...
                {filteredLanguages.length === 0 && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ textAlign: 'center', padding: '6rem 0' }}>
                        ...
                        <p style={{ fontSize: '1.2rem', color: '#6A7A7A', fontWeight: '750' }}>No localized instances found for "{searchTerm}"</p>
                    </motion.div>
                )}
                ...
                <div style={{ textAlign: 'center', marginTop: '6rem', paddingBottom: '3rem' }}>
                    <div style={{ display: 'inline-flex', alignItems: 'center', gap: '12px', opacity: 0.4 }}>
                        ...
                        <p style={{ fontSize: '1rem', color: '#B0B0B0', fontWeight: '950', letterSpacing: '0.2rem' }}>
                            ROUTEPOLY LOCALIZATION ENGINE V3.5
                        </p>
                    </div>
                </div>
            </div>
